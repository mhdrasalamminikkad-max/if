...
import { BarChart3, Users, BookOpen, Bookmark, Trash2, ArrowLeft, CheckCircle, XCircle, Clock, Plus, Mic, Square, Volume2, CreditCard, Edit, DollarSign, Coins, Ban } from "lucide-react";
...
export default function AdminPanel() {
  ...
  const { data: bannedStudents = [] } = useQuery<any[]>({
    queryKey: ["/api/students/banned"],
    enabled: adminCheck?.isAdmin || false,
  });
  ...
  const handleUnbanStudent = (regNumber: string) => {
    unbanStudentMutation.mutate(regNumber);
  };
  ...
  const unbanStudentMutation = useMutation({
    mutationFn: async (regNumber: string) => {
      const res = await apiRequest("PATCH", `/api/students/${regNumber}/ban`, { isBanned: false });
      return await res.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/students/banned"] });
      toast({
        title: "Student unbanned",
        description: "The student has been unbanned successfully.",
      });
    },
    onError: () => {
      toast({
              ...
              <TabsContent value="banned">
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>Manage banned student accounts</CardDescription>
                  </CardHeader>
                  <CardContent>
                        ...
                        <Table>
                          <TableHeader>
                            <TableRow>
                              ...
                              <TableHead>Monthly Quota (mins)</TableHead>
                          ...
                          <TableBody>
                            {bannedStudents.map((student) => (
                              <TableRow key={student.id} data-testid={`row-banned-${student.id}`}>
                                ...
                                <TableCell>{student.extraMinutes}</TableCell>
                                <TableCell>
                                  {student.isFlagged ? (
                                ...
                                <TableCell>
                                  <Button
                                    size="sm"
                                    variant="outline"
                                    onClick={() => handleUnbanStudent(student.registrationNumber)}
